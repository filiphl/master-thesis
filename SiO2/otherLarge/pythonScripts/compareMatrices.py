...
class Forces:
    ...
    def loadMatrix(self):
        ...
        print np.shape(self.absoluteForces)

        self.matrix /= self.steps

        if self.plotAverage:
            self.name='Average'
            self.fig, self.ax = plt.subplots(3,1, figsize=(5,15))
            self.plotMatrix()
    ...
    def plotMatrix(self):
        ...
        im0 = self.ax[0].imshow(self.absoluteForces, interpolation='nearest', cmap="hot_r", vmin=0, vmax=0.03)
