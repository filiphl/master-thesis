...
class SurfaceRegression:
    ...
    def leastSquarePlane(self, points):
        ...
        #normalVector = Norm([1, self.parameter[0], self.parameter[1]])
        # ai, bj, xk
        return self.Norm(parameter)
    ...
    def plotPlanes(self):
        ...
        for i in xrange(self.N):
            ...
            for j in xrange(self.N):
                ...
                Y = [j, j+1]
                X,Y = np.meshgrid(X,Y)
                surf = ax.plot_surface(X, Y, Z, rstride=1, cstride=1, cmap="gray", vmin=0, vmax=2,  antialiased=False)
                plt.hold("on")
...
if __name__ == '__main__':
    ...
    surf = SurfaceRegression(path, N=35, plot="plot" in sys.argv, nn=8)
    surf.plotPlanes()
# ---------------------------------------------------------------------------- #
