...
if args.single:
    infile.close()
    content = open(str(argv[1]), 'r').read()
    v = float(re.findall(r'vel=-([^\s]*)', content)[0])
    print 'v=%d'%v
    t = (np.asarray(slurmData['Step'])-float(min(slurmData['Step'])))*0.002 #[ps]
    d = v*t
    print len(t), len(np.asarray(slurmData[key[args.single+1]]))
    p = plt.plot(t, np.asarray(slurmData[key[args.single+1]]), linewidth=3, color=c[4])
    if key[args.single+1] == 'f_smdID[1]':
        plt.ylabel('Force [eV/\AA]', fontsize=16)
    plt.xlabel('Time [ps]', fontsize=16)
    plt.grid('on')
    plt.tick_params(axis='both', which='major', labelsize=16)
    print 'max: %.03f'%findMax(slurmData[key[args.single+1]])[0]
else:
