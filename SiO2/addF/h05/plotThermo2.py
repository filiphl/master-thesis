...
def printData(slurmData, key, v):
    Fmax = findMax(slurmData['f_smdID[1]'])[0]
...
def scale(F, v):
    F /= 0.0857*v + 0.5006                      # 130000:  u(v) = 0.0702*v + 0.3992
    return F
...
def plotV(slurmData, v, ci):
    ...
    d = v*t #AA
    F, i = cutOffTail(np.asarray(slurmData['f_smdID[1]']))
    N = np.mean(slurmData[key[-1]])
    d=d[:i]
    plt.plot(scale(d,v), scale(F,v)/N, linewidth=3, label='$v=%d$'%v, color=c[ci])
