...
class All:
    ...
    def __init__(self, dirPath='.', plotEach=False, printTable=False, plotFinal=False):
        ...
        self.findCoefficient(plotEach=plotEach, printTable=printTable, plotFinal=plotFinal)
...
if __name__ == '__main__':
    ...
    h2 = All(plotEach=0, dirPath='h2/')
    h3 = All(plotEach=0, dirPath='h3/')
    h4 = All(plotEach=0, dirPath='h4/')
    h5 = All(plotEach=0, dirPath='h5/')
    h6 = All(plotEach=0, dirPath='h6/')
    h7 = All(plotEach=0, dirPath='h7/')
    h8 = All(plotEach=0, printTable=0, plotFinal=0, dirPath='h8/')

    H = [h2, h3, h4, h5, h6, h7, h8]

    mu = [h.pf[0] for h in H]
    d  = range(2,9)
    s = max(mu)-min(mu)
    ...
    ticksFontSize = 20
    plt.grid()
    plt.ylabel(r"Coefficient of static friction", fontsize=majorFontSize, labelpad=15)
    plt.xlabel(r"Substrate thickness [7.12 \AA]", fontsize=majorFontSize, labelpad=15)
    ...
    #plt.show()
    plt.tick_params(axis='both', which='major', labelsize=ticksFontSize)
    plt.ylim([min(mu)-s*0.05, max(mu)+s*0.05])
    plt.tight_layout()
