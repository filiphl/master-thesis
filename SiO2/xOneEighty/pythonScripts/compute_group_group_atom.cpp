...
void ComputeGroupGroupAtom::pair_contribution()
{
    ...
    ilist = list->ilist;    #list of ehich index atoms has
    numneigh = list->numneigh;  # number of neighbors for each atom
    firstneigh = list->firstneigh;  #Actual neighbor list
    ...
    for (ii = 0; ii < inum; ii++) {
        ...
        for (jj = 0; jj < jnum; jj++) { #Loop over all neighbors for atom i
            j = jlist[jj];
            factor_lj = special_lj[sbmask(j)];  #spsial bonds?
