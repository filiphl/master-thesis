...
MDcalculation::MDcalculation(string path, int loadstate){
	...
	readBondFile(path + "/init/bonds.dat");
}
...
void MDcalculation::readBondFile(string filename){
	ifstream bondFile;
	bondFile.open(filename.c_str());
	if(bondFile.is_open()){
		string line;
		int nline;
		string* linesplit;
		while(getline(bondFile, line)){
			...
			for(int i = 1; i < nline; i++){
				// cout << line << "\n";
				container->addBond(stoi(linesplit[0]), stoi(linesplit[i]));
			}
		}
	}
}
...
void MDcalculation::saveInteratomicForces(string runName, int n){
	...
	sprintf(nstr, "%05d/interatomicForces_%03d.dat", n, rank);
	name.append(nstr);
	delete[] nstr;
			...
			for(Particle* p = cells[i][j][k]->getFirst(); cells[i][j][k]->hasNext(); p = cells[i][j][k]->getNext()){
				if(p->monitored){
					for(int i = 0; i < p->index2B.size(); i++){
						forceFile << p->id << " " << p->index2B[i] << " ";
						forceFile << setprecision(pres) << p->forces2B[3*i]*potential->E0 << " ";
						forceFile << setprecision(pres) << p->forces2B[3*i + 1]*potential->E0 << " ";
						forceFile << setprecision(pres) << p->forces2B[3*i + 2]*potential->E0 << "\n";
					}
				}
			}
		}
...
void MDcalculation::addNeighbour(Particle* p, Particle* q){
	//No 2BF between bonds, only bonded forces
	for(int i = 0; i < 2; i++){
		if(container->bonds[p->id][i] == -1){
			break;
		}
		if(q->id == container->bonds[p->id][i]){
			return;
		}
	}
	for(int j = 0; j < 2; j++){
		if(container->bonds[q->id][j] == -1){
			break;
		}
		if(p->id == container->bonds[q->id][j] == -1){
			return;
		}
	}
	//sharing a bond
	for(int i = 0; i < 2; i++){
		...
		for(int j = 0; j < 2; j++){
			...
			if(container->bonds[p->id][i] == container->bonds[q->id][j]){
				return;
			}
		}
	}
...
void MDcalculation::findNeighbours(){
	...
	for(int k = 0; k < Nz + 2; k++){
		...
		if(i != 0 && i != Nx + 1 && j != 0 && j != Ny + 1 && k != 0 && k != Nz + 1){
			for(Particle* p = list1->getFirst(); list1->hasNext(); p = list1->getNext()){
				if(container->bonds[p->id][0] != -1){
					p->bonds.push_back(container->particles[container->bonds[p->id][0]]);
					if(container->bonds[p->id][1] != -1){
						p->bonds.push_back(container->particles[container->bonds[p->id][1]]);
					}
				}
			}
		}else{
			...
			for(Particle* p = list1->getFirst(); list1->hasNext(); p = list1->getNext()){
				if(container->bonds[p->id][0] != -1){
					q = container->particles[container->bonds[p->id][0]];
					if(q != 0){
						p->bonds.push_back(q);
					}
					if(container->bonds[p->id][1] != -1){
						q = container->particles[container->bonds[p->id][1]];
						if(q != 0){
							p->bonds.push_back(q);
						}
					}
...
void MDcalculation::noseHoover(int Nt, double dt, double tau, double T, string runName, int saveInterval){
	dt = dt/t0;
	potential->createForceFactor(dt);
	double zeta;
	double time0, time1;
	...
	for(int i = 1; i <= Nt; i++){
		if(i%saveInterval == 0){
			zeta = integrateNoseHoover(dt, Qi, K0, zeta, monitoring);
			saveFile(runName, i/saveInterval);
			//Saving energy to file
			MPI_Allreduce(&V, &Vsum, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
			V = Vsum;
			...
			zeta = integrateNoseHoover(dt, Qi, K0, zeta, false);
		}
		if(rank == 0){
			//Print each percents completeness, needs to change! Not working
			//for values which are not a multiple of 100!
...
double MDcalculation::integrateNoseHoover(double dt, double Qi, double K0, double zeta, bool monitoring){
	int p1, p2;
	int cellx, celly, cellz;
	double K1, K2;
	...
	//Calculating forces
	calculateForces(monitoring);
	zeta = zeta + dt*Qi*0.5*(K1 + K2 - 2*K0);
	potential->createVelocityFactor(dt, zeta);
	//Calculate final velocities
	for(int i = 1; i <= Nx; i++){
	for(int j = 1; j <= Ny; j++){
	for(int k = 1; k <= Nz; k++){
