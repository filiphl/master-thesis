...
class MDcalculation{
	public:
		...
		void noseHoover(int Nt, double dt, double Q, double T, string runName, int saveInterval);
	...
	private:
		...
		double integrateNoseHoover(double dt, double Qi, double K0, double zeta, bool monitoring);
		...
		void readBondFile(string filename);
	private:
		//Processor rank
		int rank;
