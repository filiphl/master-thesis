...
class MDmanager:
	...
	path = "";
	splitted = [];
	executable = os.path.expanduser("~/usc/bin");
	...
	def setExecutable(this, executablePath):
		#
	...
	def simulate(this, length, dt, timesteps, combine = False, resume = -1):
		...
		mds.simulate(length, dt, newSimPath, timesteps = timesteps, executable = this.executable);
	...
	def thermalize(this, length, dt, tau, avglength, timesteps, combine = False, resume = -1):
		...
		mds.thermalize(length, dt, tau, T, avglength, newSimPath, timesteps = timesteps, executable = this.executable);
	...
	def noseHoover(this, length, dt, tau, timesteps, chainLength = 10, combine = False, resume = -1):
		lastSimPath = this.simulations[resume];
		lastState = this.stateNumbers[resume];
		lastSim = len(this.simulations) - 1;
		path = this.path
		...
		mds.noseHoover(length, dt, tau, T, chainLength, newSimPath, timesteps = timesteps, executable = this.executable);
	...
	def loadSimulation(this, path, length = None):
		...
		this.stateNumbers.append(length);
		this.splitted.append(True);
		this.path = "/".join((path.split("/"))[:-1]);
		datafile = this.path + "/init/data.dat";
		f = open(datafile);
		f.readline();
		s = np.array([float(val) for val in f.readline().split(' ')]);
		f.close();
		# print this.path;
	#end
	...
	def makeLens(this, r0, d, h, w = 0, direction = [1, 0, 0], invert = False, simulation = -1):
		length = this.stateNumbers[simulation];
		lastSim = this.simulations[simulation];
		splitted = this.splitted[simulation];
		path = this.path;
		...
		datafile.write("0\n0\n");
		#Creating directory to hold xyz files
		N = mdm.makeLens(lastSim, newPath, r0, h, d, w, direction, invert);
		datafile.write(str(N) + "\n");
		datafile.close();
		mdf.split(newPath, this.p, this.s, source = newPath + "/00000/combined.xyz");
		this.simulations.append(newPath);
	...
	def makeBentLens(this, r0, d, h, w = 0, direction = [1, 0, 0], simulation = -1):
		R = (h*h + d*d)/(4*d);
		this.displaceRadially(R, -np.array(direction), simulation);
		return this.makeLens(r0, d, h, w, direction, invert = False, simulation = simulation);
	#end
	
	def makeCylinder(this, r0, d, h, w = 0, direction = [1, 0, 0], invert = False, simulation = -1):
		length = this.stateNumbers[simulation];
		lastSim = this.simulations[simulation];
		splitted = this.splitted[simulation];
		path = this.path;
		...
		#end
		loadfile = lastSim + "/%05d/combined.xyz"%length;
		newPath = path + "/" + str(simNumber + 1) + "_cylinder";
		#Creating new directories and data
		this.resetRun(newPath);
		os.makedirs(newPath + "/init");
		#Create datafile
		...
		datafile.write("0\n0\n");
		#Creating directory to hold xyz files
		N = mdm.makeCylinder(lastSim, newPath, r0, h, d, w, direction, invert);
		datafile.write(str(N) + "\n");
		datafile.close();
		mdf.split(newPath, this.p, this.s, source = newPath + "/00000/combined.xyz");
		this.simulations.append(newPath);
	...
	def makeBentCylinder(this, r0, d, h, w = 0, direction = [1, 0, 0], simulation = -1):
		R = (h*h + d*d)/(4*d);
		this.displaceCylindrical(R, -np.array(direction), simulation);
		return this.makeCylinder(r0, d, h, w, direction, invert = False, simulation = simulation);
	#end
	...
	def toggleFreeze(this, simulation = -1, toggle = False):
 	...
 	def removeFreeze(this, simulation = -1):
		#Rename freeze file to unmark
		lastSim = this.simulations[simulation];
		f = lastSim + "/init/mark.dat";
		os.remove(f);
