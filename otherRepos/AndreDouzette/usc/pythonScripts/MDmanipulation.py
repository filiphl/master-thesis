...
def combine(load1, load2, save):
	...
	data2 = readfile(load2 + "/%05d/combined.xyz"%lastState2);
	N1 = len(data1);
	N2 = len(data2);
...
def makeLens(load, save, r0, h, d, w, direction, invert):
	if(not os.path.exists(save + "/00000")):
		os.makedirs(save + "/00000");
	#end
	if(not os.path.exists(save + "/init")):
	...
	for i in range(0, len(t)):
		...
		if(dx1*dx1 + dy1*dy1 + dz1*dz1 < R2 and dx2*dx2 + dy2*dy2 + dz2*dz2 < R2):
			p = r[i, :] - r0;
			if(4*(np.sum(p*p) - np.sum(p*direction)**2) < h*h):
				# print p;
				mark[i] = invert;
			#end
		#end
...
def makeCylinder(load, save, r0, h, d, w, direction, invert):
	if(not os.path.exists(save + "/00000")):
		os.makedirs(save + "/00000");
	#end
	if(not os.path.exists(save + "/init")):
	...
	R = (h*h + d*d)/(4*d);
	#Posission of the two spheres
	r1 = r0 - D*np.array([1, 0, 0]);
	r2 = r0 + D*np.array([1, 0, 0]);
	R2 = R*R;
	#Marked for deletion
	mark = np.ones(len(t))*(1 - invert);
	id = 0;
	...
	for i in range(0, len(t)):
		...
		dx2 = r[i, 0] - r2[0];
		dy2 = r[i, 1] - r2[1];
		if(dx1*dx1 + dy1*dy1 < R2 and dx2*dx2 + dy2*dy2 < R2):
			p = r[i, 1] - r0[1];
			if(4*p*p < h*h):
				mark[i] = invert;
			#end
...
def displaceCylindrical(load, save, R, direction):
	#Normalizing direction
	direction = np.array(direction);
	d = direction/np.sqrt(np.sum(direction*direction));
	R2 = R*R;
