...
\begin{abstract}
We perform molecular dynamics simulations of a single spherical asperity indenting a flat substrate.
Both bodies are made up of $\beta$-cristobalite, and we use the Vashishta potential.
The LAMMPS distribution is extended by a custom class, in order to compute contact forces.  
Radial distribution of contact forces is computed and compared to Hertz' theory, and the results of Robbins et al. \cite{RobbinsSingleAsperity}.
The static friction force is measured to be proportional to the load, in accordance with Amontons' law.
The coefficient of static friction is estimated. 
It's shown that for thin substrates it increases linearly with the substrate thickness, in agreement with recent experiments conducted by Julien Scheibert \cite{Scheibert2017}.
...
%The main goals of this thesis is to create a Molecular Dynamics model similar to that of Robbins, but keep all bodies elastic. 
...
With this model we will compute the force distribution and the coefficient of friction, before we can compare our results to those of Robbins and Scheibert. 
To accomplish this, we have posted X requirements that needs to be met:
%
...
\begin{description}
	...
	We wish to compute the spacial force distribution in the area of contact acting between the two bodies, preferably component wise. 
	...
	Secondly, we need to know the gradient of the surface, in order to define its normal and shear components.
	...
	\begin{description}
		...
		As of now, LAMMPS does not contain the functionality to retrieve the net force acting on an every individual atom of one group from all atoms of another group. A goal is therefor to develop such a method and integrate it in the LAMMPS distribution.
		\item[2) Develop method to approximate the contact surface]\hfill\\ 
		To distingush the components of the contact forces, we must approximate the surface, in order to determine a gradient.
	\end{description}
	
	\item[c) Compute the coefficient of static friction] \hfill \\
	Description of goal c).
	\item[d) Compare results to those of Robbins and Scheibert] \hfill \\
...
In collaboration with Binquan Luan, they used LAMMPS to do molecular dynamics simulations of a single spherical asperity indenting a flat substrate. 
...
figure \ref{fig:resultsRadialForceDistributions} show the radial force distribution in the area of contact, between an elastic sphere and an elastic, flat substrate. 
The distribution shows the average force acting on the sphere from the substrate within an area $a_c$.
It is therefore equivalent to the pressure distribution, we need only divide by $a_c$.  
The continuum prediction of this distribution \eqref{eq:HertzPressure} was developed by Hertz, as described in section \ref{sec:hertzTheory}. 
...
A challenge is that we do not know $E^*$, and can't compute it from equation \eqref{eq:effectiveModulus}, since $\beta$-cristobalite is not an isotropic crystal. 
However, from the resulting distribution, we may assume the radii of contact $a$ to be the (first) radial distance where $F(r)=0$, and $p_0$ to be the maximum value.
This approach result in the situation shown in figure XX.
