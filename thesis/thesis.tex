...
You, and the son we are expecting, are my greatest source of motivation.  
...
Though the force at the cut-off range is practically zero, it's not exactly zero. 
...
The $15\times15\times15$ system is sufficient for this test. The procedure is rather simple. For $\beta$-cristobalite we expect the melting point to be about 1986K\footnote{\url{https://en.wikipedia.org/wiki/Cristobalite}}, so we start out at 1500K. 
After equilibration, the following steps are repeated until we reach a target temperature. 
\begin{itemize}
...
\subsection{Observations}
 ...
 Also, as shown in figure \ref{fig:resultsRadialForceDistributions}, the standard deviation in our results is the largest near the center, where our statistical foundation is the weakest. 
...
The two curves do not coincide at the height of initial contact (indentation height = 0). 
...
However, as we can see, this issue only results in a minor discrepancy. 
...
We saw that at large radial distance, the results conform. 
...
In order to achieve this, we expanded the LAMMPS library with a custom compute, that computes per-atom forces acting between atoms of the two bodies.
...
There are many potential methodological errors that may arise in our model, and if we should continue working with these methods, it might be of interest to refine them a bit.
...
\lstinputlisting[caption={twoAtomTest.in}, label={lst:verify1}]{../SiO2/verifyCompute/tiny/allInOne.in}
\lstinputlisting[caption={twoAtomTest.data}, label={lst:verify1data}]{../SiO2/verifyCompute/tiny/system.data}
\lstinputlisting[caption={forces.txt}, label={lst:verify1Force}]{../SiO2/verifyCompute/tiny/forceFiles/forcesAll.txt}
...
\lstinputlisting[caption={forces.txt}, label={lst:verify2Force}]{../SiO2/verifyCompute/tiny2/forces.txt}
\newpage
