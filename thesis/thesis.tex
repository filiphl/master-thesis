...
A compute defines a computation that will be performed on a group of atoms. As an example the 
\begin{lstlisting}[language=LammpsInput]
compute comID groupID com
\end{lstlisting} 
computes the center of mass of all atoms within the specified \texttt{groupID}.
Their returned values are instantaneous. 
...
Examples include time evolution of atoms, i.e. updating their positions and velocities, using thermostats, applying external forces to atoms, averaging, etc. 
...
\subsubsection{fixes} can output user-specified quantities to files: fix ave/time for time averaging, fix ave/chunk for spatial or other averaging, and fix print for single-line output of variables. Fix print can also output to the screen.
\paragraph{Restart files}.
