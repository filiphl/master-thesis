...
\chapter{Computing the force distribution}
...
For solid state physics there are several algorithms which attempt at this by searching for areas with structural discrepancies and dislocations \cite{stukowski}. 
...
\subsubsection{compute\_peratom()}
 ...
 Finally it calls upon the function \texttt{pair\_contribution()}, which fills the per-atom arrays with the energy and force. 
 A very important detail is that we only use the pair-contribution to the force. 
...
\item[] \texttt{groupbit} is the 32-bit integer associated with a specific group. 
...
\end{equation}
In general there are two valid normal vectors $\vec{n}$ that satisfy the plane equation. However, the way obtain it, we always get the one with a positive z-component.  
...
In stead, we define the shear force as the radial component of the shear force as defined above.
...
\subsection{Specific description of experiment}
...
In stead of using momentary measured values for the force distributions, we present the average distribution over a time-frame of 5000 time steps (10 ps). 
...
\begin{table}[H] 
	\begin{tabular}{cr} 
		...
		-5 & \parbox[c]{0.83\linewidth}{
			\includegraphics[width=\linewidth, trim={8mm 5mm 35mm 0}, clip]{../SiO2/xOneEighty/lower/pythonScripts/timeSteps/radialOnly/timestep065000_bottom_without.pdf}
		}\\
	
		0 & \parbox[c]{0.83\linewidth}{
			\includegraphics[width=\linewidth, trim={8mm 5mm 35mm 0}, clip]{../SiO2/xOneEighty/lower/pythonScripts/timeSteps/radialOnly/timestep070000_bottom_without.pdf}
		}\\
	
		5 & \parbox[c]{0.83\linewidth}{
			\includegraphics[width=\linewidth, trim={8mm 5mm 35mm 0}, clip]{../SiO2/xOneEighty/lower/pythonScripts/timeSteps/radialOnly/timestep075000_bottom_without.pdf}
		}\\
	
		10 & \parbox[c]{0.83\linewidth}{
			\includegraphics[width=\linewidth, trim={8mm 5mm 35mm 0}, clip]{../SiO2/xOneEighty/lower/pythonScripts/timeSteps/radialOnly/timestep080000_bottom_without.pdf}
		}\\	
	
		15 & \parbox[c]{0.83\linewidth}{
			\includegraphics[width=\linewidth, trim={8mm 0 35mm 0}, clip]{../SiO2/xOneEighty/lower/pythonScripts/timeSteps/radialOnly/timestep085000_bottom.pdf}
		}\\
	\end{tabular}
\end{table}
...
\begin{table}[H] 
	\begin{tabular}{cr} 
		...
		20 & \parbox[c]{0.83\linewidth}{
			\includegraphics[width=\linewidth, trim={8mm 5mm 35mm 0}, clip]{../SiO2/xOneEighty/lower/pythonScripts/timeSteps/radialOnly/timestep090000_bottom_without.pdf}
		}\\
		
		25 & \parbox[c]{0.83\linewidth}{
			\includegraphics[width=\linewidth, trim={8mm 5mm 35mm 0}, clip]{../SiO2/xOneEighty/lower/pythonScripts/timeSteps/radialOnly/timestep095000_bottom_without.pdf}
		}\\
		
		30 & \parbox[c]{0.83\linewidth}{
			\includegraphics[width=\linewidth, trim={8mm 5mm 35mm 0}, clip]{../SiO2/xOneEighty/lower/pythonScripts/timeSteps/radialOnly/timestep100000_bottom_without.pdf}
		}\\
		
		35 & \parbox[c]{0.83\linewidth}{
			\includegraphics[width=\linewidth, trim={8mm 5mm 35mm 0}, clip]{../SiO2/xOneEighty/lower/pythonScripts/timeSteps/radialOnly/timestep105000_bottom_without.pdf}
		}\\
		
		40 & \parbox[c]{0.83\linewidth}{
			\includegraphics[width=\linewidth, trim={8mm 0 35mm 0}, clip]{../SiO2/xOneEighty/lower/pythonScripts/timeSteps/radialOnly/timestep110000_bottom.pdf}
		}\\
...
\begin{figure}[H]
	\caption{Radial distribution of contact forces acting on a sphere with radius of curvature $R=200\mathring{A}$ by a substrate of size $327.52\mathring{A} \times 327.52\mathring{A} \times 156.64\mathring{A}$ at different depths of indentation $h$. 
		...
		The radial values are given in units of the unit cell length of $\beta$-cristobalite i.e. $r=1=7.12\mathring{A}$. 
...
\end{figure}
 ...
 We notice the normal force to have its maximum value close to the center for all intervals after repulsive forces are predominant. 
 ...
 Since it isn't we can not use that argument. 
%Explenation - dip in normal force.
 The exact cause of the behavior is difficult to point out. 
 ...
 Both the sphere and the substrate are made of the same material, with the same atomic structure. 
 The sphere was simply carved out, without any consideration its structure and broken bonds. 
 It seems reasonable to expect the sphere to move slightly in order to fit best possible with the structure of the substrate, hence shifting the center of indentation from the expected position. 
 ...
 Again, at this position there are bad foundation for the statistics, so I wont read too much into this, as it is not completely systematic. 
 ...
 From Hertz' theory we would expect the radial normal force distribution to be parabolic. 
 This is not possible to conclude from the obtained results. 
 If anything, it seems linear.

 The shear force conforms with Hertz' theory much better.
...
These curves overlap quite nicely, so I do not see any cause of concern\footnote{Though this is a subjective opinon.}.
...
\begin{figure}[H]
	...
	\caption{Coeffisient of friction $\mu$ as a function of normal force/load $N$.}
	\label{fig:uvsn}
\end{figure}
