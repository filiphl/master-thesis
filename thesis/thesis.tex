...
Due to symmetry, we are only required to compute the area of a part of the domain, and not all cells. 
...
These can then be loaded, separately in order to compute the static force at different values of normal force.
Throughout the simulation, we must be certain to remain the force on the rigid top at the same value as when the restart file was written.
When lowering the sphere, we linearly increased the load. 
Because we go from a linear increase to a sudden constant value, we equilibrate a short while before measuring, in order to avert effects of pressure waves.

In order to add the shear force, we fix a spring with stiffness $k$ to the rigid top layer of the sphere cap using the \texttt{Fix smd} command
...
In the completed simulations we have used a time frame of 50000 time steps (0.1ns).
...
This should in principle only affect the resolution of the measurement, while the coefficient of static friction remain the same.
...
In turn, this means we have to extend the spring further in order to apply a high enough force. 
...
The \texttt{fix smd} has six return values: 1,2,3,4,5,6.

We are interested in the first value, i.e. the total force component in the x-direction.
By setting up a custom format for the \textit{thermo\_style}, we consistently save the value of the total force component in the x-direction every 100 time step. 
This way, we can monitor the force applied by the spring. 
...
At this point it should decrease.    
...
\begin{table}[H]
	\begin{center}
		\caption{Measurement results for simulation with substrate of thickness $h=58$\AA. 
			...
			$N$ is the measured normal force, and 
			$F_T$ the force applied to the rigid top of the sphere cap at the point slip occurs. 
			The units are as described by the $metal$ convention, shown in table \ref{tab:unitsMetal}.}
